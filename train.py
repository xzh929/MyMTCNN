    ...
    def train(self):
        ...
        while True:
            ...
            for i, (img_data_, category_, offset_) in enumerate(dataloader):  # 样本，置信度，偏移量
                ...
                sum_off_loss += offset_loss.item()
            avg_loss = sum_loss / len(dataloader)
            avg_cls_loss = sum_cls_loss / len(dataloader)
            avg_off_loss = sum_off_loss / len(dataloader)
            print("epoch=", epoch, "loss:", avg_loss, " cls_loss:", avg_cls_loss, " offset_loss", avg_off_loss)
            epoch += 1

            # 保存
            if avg_loss < ini_loss:
                ini_loss = avg_loss
